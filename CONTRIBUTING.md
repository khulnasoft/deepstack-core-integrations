# Contributing to Deepstack Core Integrations
...
> [!TIP]
> If you like Deepstack, but just don't have time to contribute, that's fine. There are other easy ways to support the project and show your appreciation, which we would also be very happy about:
> - Star this repository
> - Tweet about it
> - Mention Deepstack at local meetups and tell your friends/colleagues

## Table of Contents

- [Contributing to Deepstack Core Integrations](#contributing-to-deepstack-core-integrations)
...
By participating, you are expected to uphold this code. Please report unacceptable behavior to deepstack@khulnasoft.com.


## I Have a Question

> [!TIP]
> If you want to ask a question, we assume that you have read the available [Documentation](https://docs.deepstack.khulnasoft.com/v2.0/docs/intro).
...
[Community Channels](https://deepstack.khulnasoft.com/community), Discord in particular is often very helpful.
...
- Collect information about the bug:
  ...
  - Version of Deepstack and the integrations you're using
...
> [!IMPORTANT]
> You must never report security related issues, vulnerabilities or bugs including sensitive information to the issue tracker, or elsewhere in public. Instead sensitive bugs must be reported using [this link](https://github.com/khulnasoft/deepstack-core-integrations/security/advisories/new).
...
- Make sure that you are using the latest version.
- Read the [documentation](https://docs.deepstack.khulnasoft.com/v2.0/docs/intro) carefully and find out if the functionality is already covered, maybe by an individual configuration.
...
["good first issue"](https://github.com/khulnasoft/deepstack-core-integrations/issues?q=is%3Aopen+is%3Aissue+label%3A%22good+first+issue%22).
...
Deepstack makes heavy use of [Hatch](https://hatch.pypa.io/latest/), a Python project manager that we use to set up the
virtual environments, build the project and publish packages. As you can imagine, the first step towards becoming a
Deepstack contributor is installing Hatch. There are a variety of install methods depending on your operating system
...
```console
$ git clone https://github.com/YOUR-USERNAME/deepstack-core-integrations
```

If everything worked, you should be able to do something like this (the output might be different):

```console
$ cd deepstack-core-integrations/integrations/chroma
$ hatch version
...
> Deepstack Integrations live in the `./integrations` folder, and each one must be considered an independent project.
> All the examples shown in this section must be executed in one specific integration folder.

Hatch provides several commands we use for developing integrations, let's pick `deepstack-chroma` to show a practical
...
> [!IMPORTANT]
> Core integrations follow the naming convention `PREFIX-deepstack`, where `PREFIX` can be the name of the technology
> you're integrating Deepstack with. For example, a khulnasoft integration would be named as `khulnasoft-deepstack`.
...
Project name: khulnasoft-deepstack
Description []: An example integration, this text can be edited later

khulnasoft-deepstack
├── src
│   └── khulnasoft_deepstack
│       ├── __about__.py
│       └── __init__.py
├── tests
│   └── __init__.py
...
[documentation website](https://docs.deepstack.khulnasoft.com/v2.0/reference/integrations-chroma).
