...
from deepstack.document_stores.errors import MissingDocumentError
...
def test_namespace_init():
    with mock.patch("deepstack_integrations.document_stores.astra.astra_client.AstraDB") as client:
        AstraDocumentStore()
...
def test_to_dict():
    with mock.patch("deepstack_integrations.document_stores.astra.astra_client.AstraDB"):
        ds = AstraDocumentStore()
        result = ds.to_dict()
        assert result["type"] == "deepstack_integrations.document_stores.astra.document_store.AstraDocumentStore"
...
class TestDocumentStore(DocumentStoreBaseTests):
    ...
    def document_store(self) -> AstraDocumentStore:
        return AstraDocumentStore(
            collection_name="deepstack_integration",
