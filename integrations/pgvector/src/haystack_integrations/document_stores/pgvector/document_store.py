...
from deepstack import default_from_dict, default_to_dict
from deepstack.dataclasses.document import ByteStream, Document
from deepstack.document_stores.errors import DocumentStoreError, DuplicateDocumentError
from deepstack.document_stores.types import DuplicatePolicy
from deepstack.utils.auth import Secret, deserialize_secrets_inplace
from deepstack.utils.filters import convert
...
HNSW_INDEX_NAME = "deepstack_hnsw_index"
...
class PgvectorDocumentStore:
    ...
    def __init__(
        ...
        *,
        ...
        table_name: str = "deepstack_documents",
        ...
        A specific table to store Deepstack documents will be created if it doesn't exist yet.
        ...
        :param table_name: The name of the table to use to store Deepstack documents.
    ...
    def _create_table_if_not_exists(self):
        """
        Creates the table to store Deepstack documents if it doesn't exist yet.
        """
    ...
    def delete_table(self):
        """
        Deletes the table used to store Deepstack documents.
    ...
    def filter_documents(self, filters: Optional[Dict[str, Any]] = None) -> List[Document]:
        ...
        refer to the [documentation](https://docs.deepstack.khulnasoft.com/v2.0/docs/metadata-filtering)
        ...
        docs = self._from_pg_to_deepstack_documents(records)
        return docs
    ...
    def write_documents(self, documents: List[Document], policy: DuplicatePolicy = DuplicatePolicy.NONE) -> int:
        ...
        db_documents = self._from_deepstack_to_pg_documents(documents)
    ...
    @staticmethod
    def _from_deepstack_to_pg_documents(documents: List[Document]) -> List[Dict[str, Any]]:
        """
        Internal method to convert a list of Deepstack Documents to a list of dictionaries that can be used to insert
    ...
    @staticmethod
    def _from_pg_to_deepstack_documents(documents: List[Dict[str, Any]]) -> List[Document]:
        """
        Internal method to convert a list of dictionaries from pgvector to a list of Deepstack Documents.
        """

        deepstack_documents = []
        for document in documents:
            deepstack_dict = dict(document)
            blob_data = deepstack_dict.pop("blob_data")
            blob_meta = deepstack_dict.pop("blob_meta")
            blob_mime_type = deepstack_dict.pop("blob_mime_type")

            # postgresql returns the embedding as a string
            # so we need to convert it to a list of floats
            if document.get("embedding"):
                deepstack_dict["embedding"] = [float(el) for el in document["embedding"].strip("[]").split(",")]

            deepstack_document = Document.from_dict(deepstack_dict)

            if blob_data:
                blob = ByteStream(data=blob_data, meta=blob_meta, mime_type=blob_mime_type)
                deepstack_document.blob = blob

            deepstack_documents.append(deepstack_document)

        return deepstack_documents
