...
from deepstack.document_stores.errors import DuplicateDocumentError
from deepstack.document_stores.types import DuplicatePolicy
from deepstack.testing.document_store import CountDocumentsTest, DeleteDocumentsTest, WriteDocumentsTest
...
def test_to_dict(monkeypatch):
    ...
    assert document_store.to_dict() == {
        "type": "deepstack_integrations.document_stores.pgvector.document_store.PgvectorDocumentStore",
        "init_parameters": {
...
def test_from_deepstack_to_pg_documents():
    deepstack_docs = [
        Document(
            id="1",
    ...
    with patch(
        "deepstack_integrations.document_stores.pgvector.document_store.PgvectorDocumentStore.__init__"
    ) as mock_init:
        mock_init.return_value = None
        ds = PgvectorDocumentStore(connection_string="test")

    pg_docs = ds._from_deepstack_to_pg_documents(deepstack_docs)
...
def test_from_pg_to_deepstack_documents():
    pg_docs = [
        {
            "id": "1",
            "content": "This is a text",
    ...
    deepstack_docs = ds._from_pg_to_deepstack_documents(pg_docs)

    assert deepstack_docs[0].id == "1"
    assert deepstack_docs[0].content == "This is a text"
    assert deepstack_docs[0].dataframe is None
    assert deepstack_docs[0].blob is None
    assert deepstack_docs[0].meta == {"meta_key": "meta_value"}
    assert deepstack_docs[0].embedding == [0.1, 0.2, 0.3]
    assert deepstack_docs[0].score is None

    assert deepstack_docs[1].id == "2"
    assert deepstack_docs[1].content is None
    assert deepstack_docs[1].dataframe.equals(DataFrame({"col1": [1, 2], "col2": [3, 4]}))
    assert deepstack_docs[1].blob is None
    assert deepstack_docs[1].meta == {"meta_key": "meta_value"}
    assert deepstack_docs[1].embedding == [0.4, 0.5, 0.6]
    assert deepstack_docs[1].score is None

    assert deepstack_docs[2].id == "3"
    assert deepstack_docs[2].content is None
    assert deepstack_docs[2].dataframe is None
    assert deepstack_docs[2].blob.data == b"test"
    assert deepstack_docs[2].blob.meta == {"blob_meta_key": "blob_meta_value"}
    assert deepstack_docs[2].blob.mime_type == "mime_type"
    assert deepstack_docs[2].meta == {"meta_key": "meta_value"}
    assert deepstack_docs[2].embedding == [0.7, 0.8, 0.9]
    assert deepstack_docs[2].score is None
