...
from deepstack.dataclasses import Document, SparseEmbedding
from deepstack.testing.document_store import (
    FilterableDocsFixtureMixin,
    _random_embeddings,
)
from deepstack_integrations.components.retrievers.qdrant import (
...
class TestQdrantRetriever(FilterableDocsFixtureMixin):
    ...
    def test_to_dict(self):
        ...
        assert res == {
            "type": "deepstack_integrations.components.retrievers.qdrant.retriever.QdrantEmbeddingRetriever",
            "init_parameters": {
                "document_store": {
                    "type": "deepstack_integrations.document_stores.qdrant.document_store.QdrantDocumentStore",
                    "init_parameters": {
                        "location": ":memory:",
                        "url": None,
                        "port": 6333,
...
class TestQdrantSparseEmbeddingRetriever(FilterableDocsFixtureMixin):
    ...
    def test_to_dict(self):
        ...
        res = retriever.to_dict()
        assert res == {
            "type": "deepstack_integrations.components.retrievers.qdrant.retriever.QdrantSparseEmbeddingRetriever",
            "init_parameters": {
                "document_store": {
                    "type": "deepstack_integrations.document_stores.qdrant.document_store.QdrantDocumentStore",
                    "init_parameters": {
...
class TestQdrantHybridRetriever:
    ...
    def test_to_dict(self):
        ...
        res = retriever.to_dict()
        assert res == {
            "type": "deepstack_integrations.components.retrievers.qdrant.retriever.QdrantHybridRetriever",
            "init_parameters": {
                "document_store": {
                    "type": "deepstack_integrations.document_stores.qdrant.document_store.QdrantDocumentStore",
                    "init_parameters": {
