import numpy as np
from deepstack_integrations.document_stores.qdrant.converters import (
    convert_id,
...
def test_point_to_document_reverts_proper_structure_from_record_with_sparse():
    ...
    document = convert_qdrant_point_to_deepstack_document(point, use_sparse_embeddings=True)
...
def test_point_to_document_reverts_proper_structure_from_record_without_sparse():
    ...
    document = convert_qdrant_point_to_deepstack_document(point, use_sparse_embeddings=False)
    assert "my-id" == document.id
    assert "Lorem ipsum" == document.content
    assert "text" == document.content_type
