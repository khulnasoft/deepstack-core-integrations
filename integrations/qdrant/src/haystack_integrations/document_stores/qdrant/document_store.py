...
from deepstack.utils.filters import convert as convert_legacy_filters
...
from .converters import (
    DENSE_VECTORS_NAME,
    SPARSE_VECTORS_NAME,
    convert_deepstack_documents_to_qdrant_points,
    convert_id,
    convert_qdrant_point_to_deepstack_document,
)
...
class QdrantDocumentStore:
    ...
    def write_documents(
        ...
        with tqdm(total=len(document_objects), disable=not self.progress_bar) as progress_bar:
            for document_batch in batched_documents:
                batch = convert_deepstack_documents_to_qdrant_points(
                    document_batch,
    ...
    def get_documents_generator(
        ...
        while not stop_scrolling:
            ...
            for record in records:
                yield convert_qdrant_point_to_deepstack_document(
    ...
    def get_documents_by_id(
        ...
        for record in records:
            documents.append(
                convert_qdrant_point_to_deepstack_document(record, use_sparse_embeddings=self.use_sparse_embeddings)
            )
        return documents
    ...
    def _query_by_sparse(
        ...
        results = [
            convert_qdrant_point_to_deepstack_document(point, use_sparse_embeddings=self.use_sparse_embeddings)
            for point in points
        ]
        if scale_score:
    ...
    def _query_hybrid(
        ...
        results = [convert_qdrant_point_to_deepstack_document(point, use_sparse_embeddings=True) for point in points]
    ...
    def _set_up_collection(
        ...
        if self.use_sparse_embeddings and not has_named_vectors:
            msg = (
                ...
                f"`deepstack_integrations.document_stores.qdrant`."
            )
    ...
    def _handle_duplicate_documents(
        ...
        :param documents: A list of Deepstack Document objects.
        ...
        :param duplicate_documents: Handle duplicates document based on parameter options.
                                    ...
                                    exists.
        :returns: A list of Deepstack Document objects.
        """
