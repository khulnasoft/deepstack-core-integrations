...
from deepstack import Document, component, default_from_dict, default_to_dict
from deepstack.components.converters.utils import normalize_metadata
from deepstack.utils import Secret, deserialize_secrets_inplace
...
class UnstructuredFileConverter:
    """
    A component for converting files to Deepstack Documents using the Unstructured API (hosted or running locally).
    ...
    def __init__(
        ...
        :param document_creation_mode: How to create Deepstack Documents from the elements returned by Unstructured.
        `"one-doc-per-file"`: One Deepstack Document per file. All elements are concatenated into one text field.
        `"one-doc-per-page"`: One Deepstack Document per page.
        All elements on a page are concatenated into one text field.
        `"one-doc-per-element"`: One Deepstack Document per element. Each element is converted to a Deepstack Document.
    ...
    def run(
        ...
        """
        Convert files to Deepstack Documents using the Unstructured API.
        ...
        :returns: A dictionary with the following key:
            - `documents`: List of Deepstack Documents.
        ...
        for filepath, metadata in tqdm(
            zip(all_filepaths, meta_list), desc="Converting files to Deepstack Documents", disable=not self.progress_bar
        ):
    ...
    def _create_documents(
        ...
        """
        Create Deepstack Documents from the elements returned by Unstructured.
        """
        docs = []
