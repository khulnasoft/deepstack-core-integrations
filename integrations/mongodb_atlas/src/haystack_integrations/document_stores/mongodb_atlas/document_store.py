...
from deepstack_integrations.document_stores.mongodb_atlas.filters import _normalize_filters
...
class MongoDBAtlasDocumentStore:
    ...
    def __init__(
        ...
        *,
        ...
        self.connection: MongoClient = MongoClient(
            resolved_connection_string, driver=DriverInfo(name="MongoDBAtlasDeepstackIntegration")
        )
    ...
    def filter_documents(self, filters: Optional[Dict[str, Any]] = None) -> List[Document]:
        ...
        refer to the Deepstack [documentation](https://docs.deepstack.khulnasoft.com/v2.0/docs/metadata-filtering).
        ...
        for doc in documents:
            doc.pop("_id", None)  # MongoDB's internal id doesn't belong into a Deepstack document, so we remove it.
    ...
    def _embedding_retrieval(
        ...
        documents = [self._mongo_doc_to_deepstack_doc(doc) for doc in documents]
        return documents

    def _mongo_doc_to_deepstack_doc(self, mongo_doc: Dict[str, Any]) -> Document:
        """
        Converts the dictionary coming out of MongoDB into a Deepstack document

        :param mongo_doc: A dictionary representing a document as stored in MongoDB
        :returns: A Deepstack Document object
        """
